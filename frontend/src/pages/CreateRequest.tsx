...
const CreateRequest = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      mostraMessaggio('success', 'Richiesta creata con successo!');
      setTimeout(() => navigate('/mie-richieste'), 1500);
    } catch (err: any) {
      mostraMessaggio('error', err.response?.data?.error || 'Errore durante la creazione');
    } finally {
      setInviando(false);
    }
  };
